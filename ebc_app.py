...
def list_dir(path):
    ...
    # extinction take in the middle of conditioning.  Metadata only; nothing is decoded.
    if vids:
        try:
            meta = M.load([v["path"] for v in vids])
            ...
            order = {f: r["rank"] for f, r in rows.items()}
            ...
            for v in vids:
                ...
                for why in (M.flagged(os.path.splitext(v["name"])[0]),
                            M.derived(meta, v["name"])):
                    if why:
                        notes.append(why)
                v["notes"] = notes
